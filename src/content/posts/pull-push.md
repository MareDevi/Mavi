...
> [!tip] Tip
> **比喻：** 你（Executor）是老板，Future 是员工。 你必须主动问员工：“工作做完了吗？”（Poll）。 员工如果没做完，会记下你的电话（Waker）。等由于外部条件满足（比如文件送到了），员工打电话给你说：“老板，可以再来问我一次了。” 你只有再次去问（Poll），员工才会把结果给你。如果你不问，员工就永远坐在那里不动。
...
> [!tip] Tip
> **比喻：** 你（主线程）去餐厅点餐（启动协程）。 服务员（底层运行时）给你一个呼叫器。 当菜做好了（I/O 完成），呼叫器震动，甚至服务员直接把菜端到你桌上（Resume），推着你开始吃饭。你不需要每隔几分钟去问厨房“菜好了没”。
